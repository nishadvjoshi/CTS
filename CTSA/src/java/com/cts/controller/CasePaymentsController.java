...
public class CasePaymentsController extends HttpServlet {

    private static final long serialVersionUID = 1L;
    private static String INSERT_OR_EDIT = "/ctsCasePayments.jsp";
    private static String LIST_DAIRY_SEARCH = "/ctsCasePayments.jsp";
    private static String LIST_CASEDETAIL = "/ctsCasePayments.jsp";
    private static String HOME_PAGE = "/index.jsp";
    
    private CasePaymentsDAO dao;
    private ClientDAO clientDAO;
    
    public CasePaymentsController() {
        super();
...
try
{
        ...
        if (request.getParameter("GetDetails") != null) {
            ...
            for (String comment : selected_Comments) {
                ...
                request.setAttribute("casemasters", dao.getAllCaseMaster());
                request.getRequestDispatcher("/ctsCasePayments.jsp").forward(request, response);
            ...
            if (CasePaymentID == null || CasePaymentID.isEmpty() || CasePaymentID.equalsIgnoreCase("0")) {
                j=dao.addCasePayment(casepayment);
            } else {
                casepayment.setCcp_ID(Integer.parseInt(CasePaymentID));
                     i= dao.updateCasePayment(casepayment);
            }
          ...
          if(i>0)
               {
                     ...
                     out.println("<script type=\"text/javascript\">");
                      out.println("alert('Case Payment updated Successfully');");
                     
                        
              
                out.println("location='CasePaymentsController?action=listPayments';");
                    out.println("</script>");
               }
          ...
          if(j>0)
               {
                     ...
                     out.println("<script type=\"text/javascript\">");
                      out.println("alert('Case Payment added Successfully');");
